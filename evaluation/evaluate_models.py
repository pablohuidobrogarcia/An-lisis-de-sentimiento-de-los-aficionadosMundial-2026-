...
from src.config import EVAL_SAMPLE_SIZE, EVAL_OUTPUT_FILE, PROCESSED_DIR
from src.sentiment import predict_sentiment, predict_batch
from src.utils import load_dataframe, setup_logger, save_dataframe
...
def generate_labeling_template(output_path: str = EVAL_OUTPUT_FILE) -> None:
    ...
    if not path.exists():
        logger.error(
            "Sentiment data not found at %s. Run the pipeline first.", path,
        )
        return
    ...
    sample = (
        ...
        .apply(lambda x: x.sample(min(len(x), max(1, EVAL_SAMPLE_SIZE // 6)), random_state=42))
...
def evaluate_against_manual(labels_path: str = EVAL_OUTPUT_FILE) -> None:
    ...
    if not Path(labels_path).exists():
        logger.error("Labels file not found: %s. Run --generate-template first.", labels_path)
        return
